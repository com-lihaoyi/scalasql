...
/**
 * Models a set of live [[Sql]] expressions which need to be rendered;
 * [[Sql]] expressions not in this set can be skipped during rendering
 ...
 */
class LiveSqlExprs(values: Option[Set[Sql.Identity]]) {
  def map(f: Set[Sql.Identity] => Set[Sql.Identity]) = new LiveSqlExprs(values.map(f))
  def isLive(e: Sql.Identity) = values.fold(true)(_.contains(e))
}

object LiveSqlExprs {
  def some(v: Set[Sql.Identity]) = new LiveSqlExprs(Some(v))
