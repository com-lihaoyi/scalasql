...
class AggOps[T](v: Aggregatable[T])(implicit qr: Queryable.Row[T, _], dialect: DialectTypeMappers) {
  ...
  /** Counts the rows */
  def size: Db[Int] = v.aggregateExpr(expr => implicit ctx => sql"COUNT(1)")

  /** Computes the sum of column values */
  def sumBy[V: Numeric: TypeMapper](f: T => Db[V])(
      implicit qr: Queryable.Row[Db[V], V]
  ): Db[V] = v.aggregateExpr(expr => implicit ctx => sql"SUM(${f(expr)})")

  /** Finds the minimum value in a column */
  def minBy[V: TypeMapper](f: T => Db[V])(
      implicit qr: Queryable.Row[Db[V], V]
  ): Db[V] = v.aggregateExpr(expr => implicit ctx => sql"MIN(${f(expr)})")

  /** Finds the maximum value in a column */
  def maxBy[V: Numeric: TypeMapper](f: T => Db[V])(
      implicit qr: Queryable.Row[Db[V], V]
  ): Db[V] = v.aggregateExpr(expr => implicit ctx => sql"MAX(${f(expr)})")

  /** Computes the average value of a column */
  def avgBy[V: Numeric: TypeMapper](f: T => Db[V])(
      implicit qr: Queryable.Row[Db[V], V]
  ): Db[V] = v.aggregateExpr(expr => implicit ctx => sql"AVG(${f(expr)})")

  /** Computes the sum of column values */
  def sumByOpt[V: Numeric: TypeMapper](f: T => Db[V])(
      implicit qr: Queryable.Row[Db[V], V]
  ): Db[Option[V]] = v.aggregateExpr(expr => implicit ctx => sql"SUM(${f(expr)})")

  /** Finds the minimum value in a column */
  def minByOpt[V: Numeric: TypeMapper](f: T => Db[V])(
      implicit qr: Queryable.Row[Db[V], V]
  ): Db[Option[V]] = v.aggregateExpr(expr => implicit ctx => sql"MIN(${f(expr)})")

  /** Finds the maximum value in a column */
  def maxByOpt[V: Numeric: TypeMapper](f: T => Db[V])(
      implicit qr: Queryable.Row[Db[V], V]
  ): Db[Option[V]] = v.aggregateExpr(expr => implicit ctx => sql"MAX(${f(expr)})")

  /** Computes the average value of a column */
  def avgByOpt[V: Numeric: TypeMapper](f: T => Db[V])(
      implicit qr: Queryable.Row[Db[V], V]
  ): Db[Option[V]] = v.aggregateExpr(expr => implicit ctx => sql"AVG(${f(expr)})")
