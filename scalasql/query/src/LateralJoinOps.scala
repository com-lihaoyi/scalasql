...
import scalasql.core.{JoinNullable, Queryable, Db, WithSqlExpr}
...
class LateralJoinOps[C[_, _], Q, R](wrapped: JoinOps[C, Q, R] with Joinable[Q, R])(
   ...
   * Only supported by Postgres and MySql
   */
  def joinLateral[Q2, R2, QF, RF](other: Q => Joinable[Q2, R2])(on: (Q, Q2) => Db[Boolean])(
      implicit ja: JoinAppend[Q, Q2, QF, RF]
  ): C[QF, RF] =
   ...
   */
  def joinLateral[Q2, R2](on: Q => Db[Boolean]): FlatJoin.Mapper[Q, Q2, R, R2] = {
    val (from, expr) = Joinable.toFromExpr(wrapped)
   ...
   */
  def leftJoinLateral[Q2, R2](on: Q => Db[Boolean]): FlatJoin.NullableMapper[Q, Q2, R, R2] = {
    val (from, expr) = Joinable.toFromExpr(wrapped)
