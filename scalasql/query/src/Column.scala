...
import scalasql.core.{Context, Db, SqlStr, TypeMapper}
import scalasql.core.SqlStr.SqlStringSyntax

/**
 * A variant of [[Db]] representing a raw table column; allows assignment in updates
 * and inserts
 */
class Column[T](tableRef: TableRef, val name: String)(implicit val mappedType: TypeMapper[T])
    extends Db[T] {
  def :=(v: Db[T]) = Column.Assignment(this, v)
...
object Column {
  case class Assignment[T](column: Column[T], value: Db[T])
}
