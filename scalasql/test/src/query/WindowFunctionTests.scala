...
trait WindowFunctionTests extends ScalaSqlSuite {
  ...
  def tests = Tests {
    ...
    test("rowNumber") - checker(
      query = Text {
        Purchase.select
          .map(p => (p.shippingInfoId, p.total, rowNumber().over.partitionBy(p.shippingInfoId).sortBy(p.total).asc))
      },
      sql = """
        SELECT
          purchase0.shipping_info_id AS res__0,
    ...
    test("denseRank") - checker(
      query = Text {
        Purchase.select
          .map(p => (p.shippingInfoId, p.total, denseRank().over.partitionBy(p.shippingInfoId).sortBy(p.total).asc))
      },
      sql = """
        SELECT
          purchase0.shipping_info_id AS res__0,
    ...
    test("percentRank") - checker(
      query = Text {
        Purchase.select
          .map(p => (p.shippingInfoId, p.total, percentRank().over.partitionBy(p.shippingInfoId).sortBy(p.total).asc))
      },
      sql = """
        SELECT
          purchase0.shipping_info_id AS res__0,
    ...
    test("cumeDist") - checker(
      query = Text {
        Purchase.select
          .map(p => (p.shippingInfoId, p.total, cumeDist().over.partitionBy(p.shippingInfoId).sortBy(p.total).asc))
      },
      sql = """
        SELECT
          purchase0.shipping_info_id AS res__0,
    ...
    test("ntile") - checker(
      query = Text {
        Purchase.select
          .map(p => (p.shippingInfoId, p.total, ntile(3).over.partitionBy(p.shippingInfoId).sortBy(p.total).asc))
      },
      sql = """
        SELECT
          purchase0.shipping_info_id AS res__0,
    ...
    test("lag") - checker(
      query = Text {
        Purchase.select
          .map(p => (p.shippingInfoId, p.total, lag(p.total, 1, -1.0).over.partitionBy(p.shippingInfoId).sortBy(p.total).asc))
      },
      sql = """
        SELECT
          purchase0.shipping_info_id AS res__0,
    ...
    test("lead") - checker(
      query = Text {
        Purchase.select
          .map(p => (p.shippingInfoId, p.total, lead(p.total, 1, -1.0).over.partitionBy(p.shippingInfoId).sortBy(p.total).asc))
      },
      sql = """
        SELECT
          purchase0.shipping_info_id AS res__0,
    ...
    test("firstValue") - checker(
      query = Text {
        Purchase.select
          .map(p => (p.shippingInfoId, p.total, firstValue(p.total).over.partitionBy(p.shippingInfoId).sortBy(p.total).asc))
      },
      sql = """
        SELECT
          purchase0.shipping_info_id AS res__0,
    ...
    test("lastValue") - checker(
      query = Text {
        Purchase.select
          .map(p => (p.shippingInfoId, p.total, lastValue(p.total).over.partitionBy(p.shippingInfoId).sortBy(p.total).asc))
      },
      sql = """
        SELECT
          purchase0.shipping_info_id AS res__0,
    ...
    test("nthValue") - checker(
      query = Text {
        Purchase.select
          .map(p => (p.shippingInfoId, p.total, nthValue(p.total, 2).over.partitionBy(p.shippingInfoId).sortBy(p.total).asc))
      },
      sql = """
        SELECT
          purchase0.shipping_info_id AS res__0,
