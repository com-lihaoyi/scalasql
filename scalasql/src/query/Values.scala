...
class Values[T: TypeMapper](val ts: Seq[T])(implicit val qr: Queryable.Row[Expr[T], T]) extends Select[Expr[T], T]{
  ...
  override def map[Q2, R2](f: Expr[T] => Q2)(implicit qr: Queryable.Row[Q2, R2]) = simpleFrom().map(f)
  ...
  def aggregate[E, V](f: SelectProxy[Expr[T]] => E)(implicit qr: Queryable.Row[E, V]) = simpleFrom().aggregate(f)
