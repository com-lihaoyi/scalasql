...
class AggOps[T](v: Aggregatable[T])(implicit qr: Queryable.Simple[T, _]) {

  /** Counts the rows */
  def size: Expr[Int] = v.queryExpr(expr => implicit ctx => sql"COUNT(1)")

  /** Computes the sum of column values */
  def sumBy[V: Numeric: MappedType](f: T => Expr[V])(
