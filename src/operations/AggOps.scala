...
class AggOps[T](v: Aggregatable[T])(implicit qr: Queryable[T, _]) {
  ...
  def sumBy[V: Numeric](f: T => Expr[V])(implicit qr: Queryable[Expr[V], V]): Expr[V] =
    v.queryExpr(expr => implicit ctx => usql"SUM(${f(expr)})")

  /** Finds the minimum value in a column */
  def minBy[V: Numeric](f: T => Expr[V])(implicit qr: Queryable[Expr[V], V]): Expr[V] =
    v.queryExpr(expr => implicit ctx => usql"MIN(${f(expr)})")

  /** Finds the maximum value in a column */
  def maxBy[V: Numeric](f: T => Expr[V])(implicit qr: Queryable[Expr[V], V]): Expr[V] =
    v.queryExpr(expr => implicit ctx => usql"MAX(${f(expr)})")

  /** Computes the average value of a column */
  def avgBy[V: Numeric](f: T => Expr[V])(implicit qr: Queryable[Expr[V], V]): Expr[V] =
    v.queryExpr(expr => implicit ctx => usql"AVG(${f(expr)})")

  /** TRUE if any value in a set is TRUE */
  def any(f: T => Expr[Boolean]): Expr[Boolean] =
    v.queryExpr(expr => implicit ctx => usql"ANY(${f(expr)})")

  /** TRUE if all values in a set are TRUE */
  def all(f: T => Expr[Boolean]): Expr[Boolean] =
    v.queryExpr(expr => implicit ctx => usql"ALL(${f(expr)})")
